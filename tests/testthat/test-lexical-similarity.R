...
test_that("TF-IDF, BM25 and exact scores remain between zero and one", {
  ...
  release <- build_database_release(create_test_articles(),
  ...
  scores <- list(tfidf = similR:::tfidf_similarity(query, dimension),
                 bm25 = similR:::bm25_similarity(query, dimension),
                 exact = similR:::exact_match_score(query, dimension))
  
  expect_length(scores$tfidf,
                nrow(dimension$tfidf))
  
  expect_false(anyNA(scores$tfidf))
  
  expect_true(all(is.finite(scores$tfidf)))
